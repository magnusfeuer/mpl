...
import logging
import json
# from mpl_parser import parse_tree
...
#    mpl_grammar = resource_bytes('mpl_parser', 'mpl.lark').decode('utf-8')
    lark_parser = Lark(mpl_grammar, start="root", parser="lalr", debug=True)

    for mpl_file_name in remainder:
        with open(mpl_file_name) as f:
            mpl_text = f.read()
...
#        print(f"{json.dumps(dict_tree, indent=2)}")
        print("-------------------------")
#        svc_tree = parse_tree.convert_mpl_tree(tree)
